...
#define MAXPC (MAXPROCPAGES*PAGESIZE) /* largest ProgramCounter value */ 

struct pentry { //page table entry 
    long active; 
    long pc; //simulated program counter for the process. s
    long npages; //number of pages
    long pages[MAXPROCPAGES]; /* 0 if not allocated, 1 if allocated for each page. */ 
};
