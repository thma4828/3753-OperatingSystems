		    ...
		    if(page <= max_npages){
		    	...
		    	if(!q[proc].pages[page]) {
					...
					if(!pagein(proc,page)) {
					   ...
					   minpg = 0;
					   while(!swapped){
					   		//for(z=0; z<MAXPROCESSES; z++){ //double for loop in a while is very slow... no benifit from lru. ... implement min heap? 
					   			for(k=0; k<MAXPROCPAGES; k++){
					   				if(timestamps[proc][k] < timestamps[minpr][minpg]){
					   					if(k != page){ //if not this process, seems to be a speed icnrease from this.... (leaving pages in memory that may be used soon?)
					   						minpr = proc;
					   						minpg = k;
					   					}
					   				}
					   		...
					   		//}
					   		if(pageout(minpr, minpg)) //if pageout is successfull 
					   			swapped = 1;
					   }
					   break;
					}
